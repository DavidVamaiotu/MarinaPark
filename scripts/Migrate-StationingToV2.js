...
function assertSourceSchema(db) {
  ...
  if (!columns.has("data"))
    throw new Error("Source database has no legacy stationing data column");
...
function assertTargetSchema(db) {
  ...
  for (const required of ["data", "price_per_day_cents", "open_ended"]) {
    if (!columns.has(required))
      throw new Error(`Target stationing schema is missing ${required}`);
  }
...
function migratedRecord(row) {
  let legacy;
  try {
    legacy = JSON.parse(row.data);
  } catch (error) {
    throw new Error(
      `Rând de staționare corupt (cheia ${row.key}): ${error.message}`,
    );
  }
  return stationingCalculator.normalizeRecord({
    ...
    caravan: String(
      legacy.caravan || row.caravan || "Rulotă staționată",
    ).trim(),
...
function main() {
  ...
  const positional = args.filter(
    (arg) => arg !== "--dry-run" && arg !== "--replace",
  );
  if (positional.length !== 2) {
    fail(
      "Usage: node scripts/Migrate-StationingToV2.js [--dry-run] [--replace] <pre-upgrade.sqlite> <target.sqlite>",
    );
    return;
  }
  ...
  try {
    ...
    const records = source
      .prepare(
        "SELECT key, owner, caravan, start_date, end_date, data FROM stationing ORDER BY start_date, owner",
      )
      .all()
      .map(migratedRecord);
    if (!records.length)
      throw new Error("Source contains no stationing records");

    const summary = {
      source: sourcePath,
      target: targetPath,
      records: records.length,
      payments: records.reduce(
        (total, record) => total + record.paymentTransactions.length,
        0,
      ),
      stayLinks: records.reduce(
        (total, record) => total + record.stayLinks.length,
        0,
      ),
      dailyPriceCents: [
        ...new Set(records.map((record) => record.pricePerDayCents)),
      ],
    ...
    try {
      if (replace) {
        target.exec(
          "DELETE FROM stationing_payments; DELETE FROM stationing_stay_links; DELETE FROM stationing",
        );
      }
      for (const record of records) {
        if (!replace) {
          target
            .prepare("DELETE FROM stationing_payments WHERE stationing_key = ?")
            .run(record.key);
          target
            .prepare(
              "DELETE FROM stationing_stay_links WHERE stationing_key = ?",
            )
            .run(record.key);
          target
            .prepare("DELETE FROM stationing WHERE key = ?")
            .run(record.key);
        }
