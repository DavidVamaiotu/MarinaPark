...
function parseArguments(argv) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (["--database", "--dbfield", "--stationing"].includes(argument)) {
      ...
      options[argument === "--database" ? "database" : argument === "--dbfield" ? "dbField" : "stationing"] = path.resolve(value);
      index += 1;
      continue;
    }
...
function normalizeIdentity(value) {
  return normalizeText(value).toLocaleLowerCase("ro-RO").normalize("NFD").replace(/[\u0300-\u036f]/g, "");
}
...
function parseAdvance(note) {
  const match = String(note ?? "").match(/avans\s*:\s*([0-9]+(?:[.,][0-9]{1,2})?)/i);
...
function categoryUnit(category, legacy) {
  ...
  const tags = Array.isArray(legacy.clTag) ? legacy.clTag.map(normalizeText).filter(Boolean) : [];
  ...
  const id = tags[0] || `legacy-${normalizedCategory || "camera"}-${String(legacy.GUID).slice(0, 8)}`;
...
function legacyFacilities(value, nights) {
  ...
  for (const item of value) {
    ...
    for (const [rawKey, rawPrice] of Object.entries(item)) {
      ...
      facilities.push({
        key: isElectricity ? "electricitate" : `legacy-${normalizeIdentity(rawKey).replace(/[^a-z0-9]+/g, "-")}`,
...
function reservationBarTotal(items) {
  ...
  return normalizeMoney(
    items.reduce((sum, item) => {
      ...
      const fallback = Number(item?.price || 0) * quantity + (item?.hasSgr ? 0.5 * quantity : 0);
...
function paymentCoveredByExisting(existing) {
  ...
  if (normalizeMoney(existing?.settledPrice) > 0) return Math.min(oldPrice, normalizeMoney(existing.settledPrice));
  if (existing?.paid === true && oldPrice > 0) return oldPrice;
  if (normalizeMoney(existing?.deposit) > 0) return Math.min(oldPrice, normalizeMoney(existing.deposit));
...
function buildPersonIndex(existingReservations) {
  ...
  for (const reservation of existingReservations) {
    ...
    index.set(personIdentity(reservation.guest, reservation.phone), reservation.personId);
  }
  return index;
}
...
function flattenLegacyReservations(dbField) {
  if (!Array.isArray(dbField?.Parkline)) throw new Error("dbField-2.json nu conține lista Parkline.");
  return dbField.Parkline.flatMap((category) => {
    const reservations = Array.isArray(category?.Rezervari) ? category.Rezervari : [];
    return reservations.map((reservation) => ({ category: normalizeText(category.NumeArray), legacy: reservation }));
  });
}
...
function mapReservation(entry, context) {
  ...
  if (end <= start) throw new Error(`Interval invalid pentru rezervarea ${guid}: ${start} - ${end}`);
  ...
  const coveredPrice = existing ? Math.min(price, paymentCoveredByExisting(existing)) : explicitAdvance;
  const stationingGuid = normalizeText(legacy.StationareGUID);
  const stationing = stationingGuid ? context.stationingByGuid.get(stationingGuid) : null;
  ...
  const mapped = {
    ...
    actualPaidAmount: existing ? normalizeMoney(existing.actualPaidAmount) : explicitAdvance,
    paymentMethod,
...
function mapStationing(legacy, linkedReservations, context) {
  ...
  const purchasedNights = nightlyPrice > 0 ? Math.round(legacyTotal / nightlyPrice) : 0;
  const prepaidNights = Math.max(1, Math.min(1095, purchasedNights + usedNights));
  const totalPrice = nightlyPrice > 0 ? normalizeMoney(Math.max(0, prepaidNights - usedNights) * nightlyPrice) : legacyTotal;
  const outstanding = stationingOutstanding(legacy.notes);
  const explicitlyPaid = /\bachitat\b/i.test(String(legacy.notes ?? "")) && !/de\s+achitat/i.test(String(legacy.notes ?? ""));
  const paidAmount = explicitlyPaid
    ? totalPrice
    : outstanding === null
      ? 0
      : normalizeMoney(Math.max(0, totalPrice - Math.min(totalPrice, outstanding)));
  const linked = linkedReservations.get(guid) || [];
  const phone = linked.map((entry) => normalizePhone(entry.legacy.telefon)).find(Boolean) || "";
  const caravan = linked.map((entry) => normalizeText(entry.legacy.masina)).find(Boolean) || "Rulotă staționată";
  const owner = normalizeText(legacy.nume) || "Proprietar necunoscut";
  const deductions = usedNights > 0
    ? [{
...
function assertSchema(db) {
  ...
  for (const table of required) {
    if (!tables.has(table)) throw new Error(`Baza țintă nu conține tabela ${table}.`);
  }
}

function readReservationRows(db) {
  return db.prepare("SELECT key, order_index, data FROM reservations ORDER BY order_index").all().map((row) => ({
    key: row.key,
...
function barFingerprint(db) {
  return crypto.createHash("sha256").update(JSON.stringify(
    db.prepare("SELECT key, name, stock, vat_rate, updated_at, data FROM bar_articles ORDER BY key").all()
  )).digest("hex");
}

function existingReservationBarFingerprint(rows) {
  return crypto.createHash("sha256").update(JSON.stringify(
    rows.map((row) => [row.key, Array.isArray(row.data.barItems) ? row.data.barItems : []])
  )).digest("hex");
}
...
function main() {
  ...
  for (const filePath of [options.database, options.dbField, options.stationing]) {
    if (!fs.existsSync(filePath)) throw new Error(`Fișier inexistent: ${filePath}`);
  }
  ...
  const stationingEntries = Array.isArray(stationingFile?.Stationari) ? stationingFile.Stationari : null;
  if (!stationingEntries) throw new Error("Stationare.json nu conține lista Stationari.");
  assertUnique(reservationEntries.map((entry) => normalizeText(entry.legacy.GUID)), "GUID rezervare");
  assertUnique(stationingEntries.map((entry) => normalizeText(entry.StationareGUID)), "GUID staționare");
  ...
  for (const entry of reservationEntries) {
    ...
    if (!guid) continue;
    if (!stationingEntries.some((item) => normalizeText(item.StationareGUID) === guid)) {
      throw new Error(`Rezervarea ${entry.legacy.GUID} indică o staționare inexistentă: ${guid}`);
    }
  ...
  const existingOrder = new Map(existingRows.map((row) => [row.key, row.orderIndex]));
  const personIndex = buildPersonIndex(existingRows.map((row) => row.data));
  const stationingContext = { importedAt, stationingHash };
  const mappedStationing = stationingEntries.map((entry) => mapStationing(entry, linkedReservations, stationingContext));
  const stationingByGuid = new Map(mappedStationing.map((entry) => [entry.legacySource.guid, entry]));
  const reservationContext = { importedAt, dbFieldHash, existingByKey, personIndex, stationingByGuid };
  const mappedReservations = reservationEntries.map((entry) => mapReservation(entry, reservationContext));
  const insertedReservations = mappedReservations.filter((entry) => !entry.existing).length;
  const updatedReservations = mappedReservations.length - insertedReservations;
  const existingStationingKeys = new Set(db.prepare("SELECT key FROM stationing").all().map((row) => row.key));
  const insertedStationing = mappedStationing.filter((record) => !existingStationingKeys.has(record.key)).length;
  const updatedStationing = mappedStationing.length - insertedStationing;
  const unitsToInsert = new Map(mappedReservations.map((entry) => [entry.unit.id, entry.unit]));
  const existingUnitIds = new Set(db.prepare("SELECT id FROM units").all().map((row) => row.id));
  const missingUnits = [...unitsToInsert.values()].filter((unit) => !existingUnitIds.has(unit.id));
  ...
  console.log(`Rezervări legacy: ${mappedReservations.length} (${insertedReservations} noi, ${updatedReservations} actualizate)`);
  console.log(`Staționări legacy: ${mappedStationing.length} (${insertedStationing} noi, ${updatedStationing} actualizate)`);
  console.log(`Unități noi necesare: ${missingUnits.length}`);
  console.log(`Legături rezervare-staționare: ${linkedReservations.size}`);
  console.log(`Bar protejat: ${db.prepare("SELECT COUNT(*) AS count FROM bar_articles").get().count} articole`);
  ...
  let nextOrder = existingRows.reduce((maximum, row) => Math.max(maximum, row.orderIndex), -1) + 1;
  db.exec("BEGIN IMMEDIATE");
  try {
    for (const unit of missingUnits) {
      const data = { ...unit, pricingMode: "per-night", adultPrice: 0, childPrice: 0, dailyPrices: {} };
    ...
    for (const entry of mappedReservations) {
      ...
      const orderIndex = existingOrder.has(reservation.key) ? existingOrder.get(reservation.key) : nextOrder++;
  ...
  const existingRowsAfter = allRowsAfter.filter((row) => previousKeys.has(row.key));
  const reservationBarsAfter = existingReservationBarFingerprint(existingRowsAfter);
  if (barAfter !== barBefore) throw new Error("Protecția barului a eșuat: tabela bar_articles s-a modificat.");
  if (reservationBarsAfter !== reservationBarsBefore) throw new Error("Protecția barului a eșuat: produsele de bar ale unei rezervări existente s-au modificat.");
  const integrity = db.prepare("PRAGMA integrity_check").get().integrity_check;
  if (integrity !== "ok") throw new Error(`Verificarea SQLite a eșuat: ${integrity}`);
  const finalReservationCount = db.prepare("SELECT COUNT(*) AS count FROM reservations").get().count;
  const finalStationingCount = db.prepare("SELECT COUNT(*) AS count FROM stationing").get().count;
  const finalBarCount = db.prepare("SELECT COUNT(*) AS count FROM bar_articles").get().count;
  db.exec("PRAGMA wal_checkpoint(TRUNCATE)");
  db.close();

  console.log(`Backup: ${backupPath}`);
  console.log(`Rezultat: ${finalReservationCount} rezervări, ${finalStationingCount} staționări, ${finalBarCount} articole bar.`);
