...
function oauthUrlFromArgs(args = []) {
  return (
    args.find((value) =>
      String(value).startsWith(`${marinaOAuthProtocol}://`),
    ) || null
  );
}
...
function registerDesktopOAuthProtocol() {
  try {
    if (app.isPackaged) app.setAsDefaultProtocolClient(marinaOAuthProtocol);
      ...
      app.setAsDefaultProtocolClient(marinaOAuthProtocol, process.execPath, [
  ...
  try {
    ...
    const desktopFile = path.join(
      ...
      "marina-park-oauth.desktop",
    );
    const projectArgument = app.isPackaged
      ? ""
    ...
    try {
      execFileSync("update-desktop-database", [applicationsDirectory], {
        stdio: "ignore",
      });
    } catch {}
    try {
      execFileSync(
        ...
        [
          "default",
          path.basename(desktopFile),
          `x-scheme-handler/${marinaOAuthProtocol}`,
        ],
        { stdio: "ignore" },
      );
    } catch {}
    ...
    console.error(
      "Marina OAuth Linux protocol registration failed:",
      error.message,
    );
  }
}
  ...
  const ensureSecureStorage = () => {
    if (
      ...
      const error = new Error(
        "Conectarea Marina necesită stocarea securizată a sistemului.",
      );
  ...
  return {
    hasRefreshTokenSync() {
      try {
        ensureSecureStorage();
        return Boolean(
          safeStorage.decryptString(fsSync.readFileSync(tokenPath)),
        );
      } catch {
        return false;
      }
...
function syncWindowZoom() {
  ...
  if (Math.abs(mainWindow.webContents.getZoomFactor() - zoomFactor) < 0.0001)
    return;
...
async function captureMainWindowJpeg() {
  ...
  if (image.isEmpty())
    throw new Error("Fereastra Marina Park nu poate fi capturată momentan");
  ...
  return {
    ...
    pointer: {
      ...
      x: pointerVisible
        ? Math.max(0, Math.min(1, (pointer.x - bounds.x) / bounds.width))
        : 0,
      y: pointerVisible
        ? Math.max(0, Math.min(1, (pointer.y - bounds.y) / bounds.height))
        : 0,
...
async function legacyInstallCandidates(userDataDir) {
  ...
  return {
    ...
    candidates: [
      ...new Set(
        candidates.filter(Boolean).map((candidate) => path.resolve(candidate)),
      ),
    ],
  };
}
...
async function migrateLegacyFiles(userDataDir, dataDir, customDir) {
  ...
  for (const candidate of candidates) {
    ...
    if (!(await pathExists(path.join(legacyDataDir, "marina-park.sqlite"))))
      continue;
...
async function chooseLegacyFolderIfNeeded(dataDir, customDir) {
  if (
    !app.isPackaged ||
    (await pathExists(path.join(dataDir, "marina-park.sqlite")))
  )
    return;
  ...
  const answer = await dialog.showMessageBox({
    ...
    detail:
      "Poți alege folderul vechi pentru a copia baza de date. Fișierele existente nu vor fi modificate.",
...
async function createWindow(url, customDir) {
  ...
  mainWindow.webContents.on("will-navigate", (event, targetUrl) => {
    ...
    if (targetOrigin && targetOrigin === applicationOrigin) return;
...
function setupAutoUpdater() {
  ...
  autoUpdater.on("update-downloaded", async (info) => {
    const answer = await dialog.showMessageBox(mainWindow, {
      ...
      detail:
        "Datele, backupurile și fișierele personalizate rămân neschimbate.",
...
async function startApplication() {
  ...
  const { url } = await serverController.startServer({
    host: "0.0.0.0",
    localHost: "127.0.0.1",
    port: 4173,
    portAttempts: 100,
  ...
  for (const oauthUrl of pendingOAuthUrls.splice(0))
    await handleOAuthUrl(oauthUrl);
