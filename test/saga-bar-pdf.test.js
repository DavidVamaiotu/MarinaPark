...
test("SAGA bar export asks for exactly one XML or PDF format", () => {
  ...
  assert.match(app, /const endpoint = format === "pdf" \? "\/api\/saga\/bar-sales\.pdf" : "\/api\/saga\/bar-sales";/);
...
test("PDF report contains readable sales columns, totals, wrapping, and pagination support", () => {
  ...
  const xmlBuilder = server.match(/function buildSagaBarSalesXml\(options = \{\}\) \{[\s\S]*?\n\}\n\nfunction htmlEscape/)?.[0];
  const pdfBuilder = server.match(/async function buildSagaBarSalesPdf\(options = \{\}\) \{[\s\S]*?\n\}/)?.[0];
...
test("existing SAGA XML route and content type remain unchanged", () => {
  ...
  assert.match(server, /buildSagaBarSalesXml\(Object\.fromEntries\(url\.searchParams\.entries\(\)\)\)/);
